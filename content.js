...
function findMovieAndFormatElements() {
  ...
  movieListings.forEach((listing) => {
    ...
    if (!hasRequiredFormat) {
      ...
      if (format === "2d") formatPatterns = ["2d"];
      else if (format === "3d") formatPatterns = ["3d"];
      else if (format === "imax") formatPatterns = ["imax"];
      else if (format === "imax 2d") formatPatterns = ["imax 2d", "imax.*2d"];
      else if (format === "imax 3d") formatPatterns = ["imax 3d", "imax.*3d"];
      else if (format === "4dx") formatPatterns = ["4dx"];
      else if (format === "4dx 2d") formatPatterns = ["4dx 2d", "4dx.*2d"];
      else if (format === "4dx 3d") formatPatterns = ["4dx 3d", "4dx.*3d"];
      else formatPatterns = [format];
    ...
    if (hasRequiredFormat) {
      ...
      // If we couldn't find specific elements, highlight the whole listing
      if (movieNameElements.length === 0) {
        highlightElement(listing, "#ffeb3b");
      }
    }
  });

  // If we didn't find any elements with the specific structure, fall back to the general approach
  if (foundElements.length === 0) {
    // Original fallback search logic
    // Selector for elements that might contain movie title or format info
    const selector =
      "h1, h2, h3, h4, h5, p, span, a, div, li, strong, em, b, i";
    const allElements = document.querySelectorAll(selector);

    // First pass: Find elements that contain the movie name
    allElements.forEach((element) => {
      // Skip elements with no text content or very short content
      if (!element.innerText || element.innerText.trim().length < 2) return;

      const elementText = element.innerText.toLowerCase();

      // Check if element contains the movie name (either full name or significant parts)
      let containsMovieName = false;

      if (elementText.includes(movieNameToFind.toLowerCase())) {
        containsMovieName = true;
      } else {
        // Check if element contains enough of the search terms (for partial matches)
        // For longer movie names, we'll require multiple terms to match
        let matchCount = 0;
        searchTerms.forEach((term) => {
          if (term.length > 2 && elementText.includes(term)) {
            matchCount++;
          }
        });

        // If it's a multi-word movie name, require at least half the terms to match
        if (
          searchTerms.length > 1 &&
          matchCount >= Math.max(2, Math.floor(searchTerms.length / 2))
        ) {
          containsMovieName = true;
        }
      }

      if (containsMovieName) {
        // Check if the same element or its immediate parent/children also contain the format
        let hasFormat = formatPreferenceToFind === "ANY";

        if (!hasFormat) {
          const formatLower = formatPreferenceToFind.toLowerCase();

          // Check self
          if (elementText.includes(formatLower)) {
            hasFormat = true;
          }
          ...
          if (
            ...
            element.parentElement.innerText.toLowerCase().includes(formatLower)
          ) {
            hasFormat = true;
          }

          // Check siblings
          if (!hasFormat && element.parentElement) {
            Array.from(element.parentElement.children).some((sibling) => {
              if (
                sibling !== element &&
                sibling.innerText &&
                sibling.innerText.toLowerCase().includes(formatLower)
              ) {
                hasFormat = true;
                return true;
              }
        ...
        if (hasFormat) {
          ...
          highlightElement(element, "#ffeb3b"); // Yellow highlight for movie name
          foundFormat = true;
          formatInfo =
            formatPreferenceToFind === "ANY"
              ? "detected format"
